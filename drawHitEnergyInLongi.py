...
def drawEvent(evt):
    ...
    hitsDensity = calHitsTree.kernel_density(caloHitsArray[:], h=10, kernel='gaussian')
    ...
    #thDensity = [0.8, 2, 3, 5]
    #thDensity = [0.6, 1.5, 3, 6]
    thDensity = [0.6, 0.7, 0.8, 1]
    ...
    for threshold in thDensity: 
        ...
        plt.hist(yDenArr, weights=eDenArr, range=(1800, 2000), bins=10, color='y')
        #plt.hist(yDenArr, range=(1800, 2000), bins=10, color='b')
    ...
    fig = plt.gcf()
    fig.savefig('showerPlot.pdf', format='pdf', dpi=1000)
